...
function operate(fn) {
    return function() {
        ...
        var handlers = args[0].handlers;
        var cbIndex = args.length - 1;
        var cb = args[cbIndex];

        args[cbIndex] = function(err, d) {
            // TODO: figure out the right pattern to apply here
            cb(err, d);
            evaluateHandlers(handlers.post, funkit.noop, d);
        };

        evaluateHandlers(handlers.pre, function() {
